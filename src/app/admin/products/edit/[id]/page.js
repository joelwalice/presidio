...
const EditProduct = () => {
    ...
    useEffect(() => {
        if (!id) {
            ...
            const fetchData = async () => {
                try {
                    console.log(id);
                    const response = await axios.get(`${process.env.NEXT_PUBLIC_BASE_API_URL}/api/products/${id}`);
    ...
    const createP = async (e) => {
        ...
        await axios.put(`${process.env.NEXT_PUBLIC_BASE_API_URL}/api/products/${id}`, {
            image: image,
            name: name,
            category: category,
            sku: sku,
